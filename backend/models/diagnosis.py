...
class DiagnosisReport(BaseModel):
    ...
    def generate_learning_path(self):
        """生成个性化学习路径"""
        from models.knowledge import KnowledgePoint
        # KnowledgeRelation 暂时注释，等待定义
        # from models.knowledge import KnowledgeRelation
        ...
        for gap in sorted_gaps[:5]:  # 取前5个最需要改进的知识点
            ...
            if kp:
                # 查找前置知识点 - 暂时注释，等待KnowledgeRelation定义
...
class DiagnosisSession(BaseModel):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    diagnosis_report_id = db.Column(db.String(36), db.ForeignKey('diagnosisreport.id'), nullable=False)
...
class QuestionResponse(BaseModel):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    diagnosis_session_id = db.Column(db.String(36), db.ForeignKey('diagnosissession.id'), nullable=False)
